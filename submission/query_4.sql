...
streaked AS (
    SELECT
        *,
        SUM(CASE WHEN is_active <> COALESCE(is_active_last_year, FALSE) THEN 1  ELSE 0 END)
        OVER (PARTITION BY actor_id ORDER BY current_year)
        AS streak_active,
        SUM(CASE WHEN quality_class <> COALESCE(quality_class_last_year, '') THEN 1 ELSE 0 END)
        OVER (PARTITION BY actor_id ORDER BY current_year)
        AS streak_qc
    FROM lagged)
